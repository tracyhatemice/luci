...
// AmneziaWG obfuscation parameters: UCI option name -> config key.
...
return network.registerProtocol('amneziawg', {
	...
	renderFormOptions(s) {
		...
		try {
			s.tab('peers', _('Peers'), _('Further information about AmneziaWG interfaces and peers at %s.'.format('<a href=\'https://amnezia.org\'>amnezia.org</a>')));
		}
		catch(e) {}
		...
		ss.handleConfigImport = function(mode) {
			...
			const nodes = E('div', {
				...
				E([], (mode == 'full') ? [
					...
					E('p', _('Paste or drag an AmneziaWG configuration (commonly <em>awg0.conf</em>) from another system below to create a matching peer entry allowing that system to connect to the local AmneziaWG interface.')),
